...
\textbf{Formulation:}
We replace the dynamics constraints in MPC with the data constraint.
\[
	\min_{\vec{z}, \vec{w}} \quad \|\vec{z}\|_{\mat{Q}}^2 + \lambda_g \|\vec{w}\|_2^2 + \lambda_y \|\mat{\sigma}_y\|_2^2
\]
subject to:
\begin{align*}
	\begin{bmatrix} \vec{u}_{past} \\ \vec{y}_{past} \\ \vec{u}_{future} \\ \vec{y}_{future} \end{bmatrix} &= \mat{H} \vec{w} + \begin{bmatrix} \vec{0} \\ \mat{\sigma}_y \\ \vec{0} \\ \vec{0} \end{bmatrix} \\
	\vec{u}_{past} &= \vec{u}_{\text{measured}} \\
	\vec{y}_{past} &= \vec{y}_{\text{measured}} \\
	\vec{z} &= (\vec{y}_{future}, \vec{u}_{future})
\end{align*}
Here, \(\vec{w}\) is the decision variable that selects the trajectory from the data library. We regularize \(\vec{w}\) to avoid overfitting to noise.
