...
\begin{itemize}
	...
	\item \textbf{Multiplication:} Quaternion multiplication \(\vec{q}_a \otimes \vec{q}_b\) (which corresponds to composing rotations) is defined as:
	      ...
	      \]
	      This can be written in matrix form as \(\mat{L}(\vec{q}_a)\vec{q}_b\) or \(\mat{R}(\vec{q}_b)\vec{q}_a\), where:
        \[
        \mat{L}(\vec{q}) = \begin{bmatrix} s & -\vec{v}^\top \\ \vec{v} & s\mat{I} + \hat{\vec{v}} \end{bmatrix}, \quad
        \mat{R}(\vec{q}) = \begin{bmatrix} s & -\vec{v}^\top \\ \vec{v} & s\mat{I} - \hat{\vec{v}} \end{bmatrix}
        \]
...
\end{code}
\begin{note}
  Note that optimisation w.r.t.\  to qwuaterinions and rotation matrixes are often non-convex, as the space of valid soluitions is a shell of unit sphere, hence the the set ofg solutions is non-convex in lature. But often these roatations optimisation problems admit strong convex relaxations. In particular you can show that when dealing with linear constraints, tthe solution will be pushed to the bundary of the set, allowing for converting the unit norm consstraint to a unit ball one. Wahba's problem is one such example.
\end{note}
\newpage
