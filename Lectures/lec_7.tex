...
\begin{equation}
    \mathcal{L}(\vec{X}, \vec{U}, \vec{\Lambda}) = H_1(\vec{x}_1, \vec{u}_1, \vec{\lambda}_2) + \sum_{k=2}^{N-1} H_k(\vec{x}_k, \vec{u}_k, \vec{\lambda}_{k+1}) + l_F(\vec{x}_N) - \lambda_N^{\top} \vec{x}_N
\end{equation}
...
\[
    \nabla_{\vec{u}_k} J = (\nabla_{\vec{u}_k} \mathcal{L})^{\top} = (\nabla_{\vec{u}} H_k)^{\top} = \mat{R}_k \vec{u}_k + \mat{B}_k^{\top} \vec{\lambda}_{k+1}
\]
