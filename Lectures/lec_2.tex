...
\begin{definition}[Equilibrium Point]
	An equilibrium point (or fixed point) \(\vec{x}^*\) of a continuous-time system \(\dot{\vec{x}} = f(\vec{x}, \vec{u})\) for a constant control input \(\vec{u}^*\) is a state that satisfies:
	\begin{equation}
		f(\vec{x}^*, \vec{u}^*) = \vec{0}
	\end{equation}
	This means that if the system starts at \(\vec{x}^*\) with control \(\vec{u}^*\), its state derivative is zero, and it will not move.
\end{definition}
...
\begin{eg}[Pendulum Equilibria]
	...
	\begin{itemize}
		\item \(\vec{x}^*_1 = [0, 0]^{\top}\): The pendulum is hanging straight down, at rest.
		\item \(\vec{x}^*_2 = [\pi, 0]^{\top}\): The pendulum is balanced perfectly upright, at rest.
	\end{itemize}
\end{eg}

A basic control problem is to create a new equilibrium point. For instance, can we make the pendulum hang at \(\theta = \pi/2\)? We need to find a constant control \(u^*\) such that \(f([\pi/2, 0]^{\top}, u^*) = \vec{0}\).
\[
	\dot{\vec{x}} = \begin{bmatrix} 0 \\ \frac{1}{ml^2}(u^* - mgl\sin(\pi/2)) \end{bmatrix} = \begin{bmatrix} 0 \\ 0 \end{bmatrix}
\]
...
\begin{intuition}[1D Systems]
	...
	\end{center}
	An equilibrium \(x^*\) is \textbf{locally stable} if, for points \(x\) near \(x^*\), the dynamics push the state back towards \(x^*\). This happens when the slope of \(f(x)\) at \(x^*\) is negative.
	\begin{itemize}
		\item If \(\pdv{f}{x}|_{x^*} < 0\), the equilibrium is stable.
		\item If \(\pdv{f}{x}|_{x^*} > 0\), the equilibrium is unstable.
		\item If \(\pdv{f}{x}|_{x^*} = 0\), the analysis is inconclusive (marginally stable).
	\end{itemize}
\end{intuition}

This concept generalizes to higher dimensions. For a system \(\dot{\vec{x}} = f(\vec{x})\), we linearize the dynamics around an equilibrium \(\vec{x}^*\). Let \(\delta\vec{x} = \vec{x} - \vec{x}^*\).
\[
	\delta\dot{\vec{x}} \approx \left.\pdv{f}{\vec{x}}\right|_{\vec{x}^*} \delta\vec{x} = \mat{A} \delta\vec{x}
\]
The stability of the nonlinear system near \(\vec{x}^*\) is determined by the stability of this linear system.

\begin{theorem}[Stability by Linearization]
	An equilibrium point \(\vec{x}^*\) of \(\dot{\vec{x}} = f(\vec{x})\) is:
	\begin{itemize}
		\item \textbf{Asymptotically Stable} if all eigenvalues of the Jacobian \(\mat{A} = \pdv{f}{\vec{x}}|_{\vec{x}^*}\) have strictly negative real parts (\(\text{Re}(\lambda_i) < 0\)).
...
\begin{eg}[Pendulum Stability Analysis]
	...
	\]
	\begin{enumerate}
		\item \textbf{At \(\vec{x}^*_1 = [0, 0]^{\top}\) (hanging down)}:
		      \[
		...
		\item \textbf{At \(\vec{x}^*_2 = [\pi, 0]^{\top}\) (upright)}:
		      \[
...
Consider an autonomous system \(\dot{\vec{x}} = f(\vec{x})\) with an equilibrium \(\vec{x}^*\) (i.e. \(f(\vec{x}^*)=0\)). Define the error \(\vec{e} = \vec{x} - \vec{x}^*\) and rewrite about the origin (w.l.o.g. take \(\vec{x}^* = \vec{0}\) by translation). We recall several nested stability notions.
...
\begin{definition}[Basin (Region) of Attraction]
	For an asymptotically stable equilibrium \(\vec{x}^*\), its (open) basin of attraction \(\mathcal{B}(\vec{x}^*)\) is the set of initial conditions whose trajectories converge to \(\vec{x}^*\):
	\[
		\mathcal{B}(\vec{x}^*) := \{\vec{x}_0 \in \mathbb{R}^n : \lim_{t\to\infty} \phi_t(\vec{x}_0) = \vec{x}^*\},
	\]
	where \(\phi_t\) denotes the flow map. If \(\mathcal{B}(\vec{x}^*) = \mathbb{R}^n\), the equilibrium is globally asymptotically stable.
\end{definition}
