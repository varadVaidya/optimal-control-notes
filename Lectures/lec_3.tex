...
A fundamental problem in numerical methods is root finding: for a given function \(g: \mathbb{R}^n \to \mathbb{R}^n\), find a point \(\vec{x}^\star\) such that \(g(\vec{x}^\star) = \vec{0}\).
...
A closely related problem is finding a \textbf{fixed point}, i.e., an \(\vec{x}^\star\) such that \(g(\vec{x}^\star) = \vec{x}^\star\). We can always convert a root-finding problem \(r(\vec{x}) = 0\) to a fixed-point problem by defining \(g(\vec{x}) = \vec{x} - r(\vec{x})\).
...
A \textbf{first-order necessary condition} for a point \(\vec{x}^\star\) to be a local minimum is that the gradient vanishes:
\begin{equation}
...
However, there are pitfalls. The condition \(\nabla f(\vec{x}^\star) = \vec{0}\) applies to local maxima and saddle points as well as minima. A standard Newton step can easily converge to the wrong type of point.

A \textbf{second-order sufficient condition} for \(\vec{x}^\star\) to be a strict local minimum is that \(\nabla f(\vec{x}^\star) = \vec{0}\) and the Hessian \(\nabla^2 f(\vec{x}^\star)\) is \textbf{positive definite} (all its eigenvalues are positive).
...
Let \(f\in C^2\). A point \(\vec{x}^\star\) is a (local) minimizer only if the following hold.
\begin{theorem}[First-Order Necessary Condition]
If \(\vec{x}^\star\) is a local minimizer, then \(\nabla f(\vec{x}^\star) = \vec{0}.\)
\end{theorem}

\begin{theorem}[Second-Order Necessary Condition]
If \(\vec{x}^\star\) is a local minimizer and \(f\in C^2\), then \(\nabla f(\vec{x}^\star) = 0\) and \(\nabla^2 f(\vec{x}^\star)\) is PSD.
\end{theorem}

\begin{theorem}[Second-Order Sufficient Condition]
If \(\nabla f(\vec{x}^\star) = 0\) and \(\nabla^2 f(\vec{x}^\star)\) is positive definite (PD), then \(\vec{x}^\star\) is a strict local minimizer. If \(\nabla^2 f(\vec{x}) \succeq m\mat{I}\) globally (\(m>0\)), then \(\vec{x}^\star\) is the unique global minimizer.
\end{theorem}
